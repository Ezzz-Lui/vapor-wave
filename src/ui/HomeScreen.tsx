import { META } from '../game/config/gameConfig'

interface HomeScreenProps {
  highScore: number
  musicEnabled: boolean
  onToggleMusic: () => void
  onStart: () => void
}

export function HomeScreen({
  highScore,
  musicEnabled,
  onToggleMusic,
  onStart,
}: HomeScreenProps) {
  return (
    <section className="home-screen" aria-label="Pantalla de inicio">
      <div className="home-panel">
        <p className="home-kicker">ENDLESS RUNNER</p>
        <h1 className="home-title">{META.title}</h1>

        <p className="home-high-score">
          HIGH SCORE <span>{highScore}</span>
        </p>

        <div className="home-actions">
          <button type="button" className="home-start" onClick={onStart}>
            Empezar partida
          </button>

          <button
            type="button"
            className="home-setting"
            onClick={onToggleMusic}
            aria-pressed={musicEnabled}
          >
            Música: {musicEnabled ? 'Activada' : 'Desactivada'}
          </button>
        </div>

        <a
          className="home-repo"
          href={META.githubUrl}
          target="_blank"
          rel="noreferrer"
        >
          <svg
            className="home-repo-icon"
            viewBox="0 0 24 24"
            aria-hidden="true"
          >
            <path
              fill="currentColor"
              d="M12 2C6.48 2 2 6.58 2 12.26c0 4.52 2.87 8.35 6.84 9.71.5.1.68-.22.68-.49 0-.24-.01-.88-.01-1.73-2.78.62-3.37-1.37-3.37-1.37-.45-1.18-1.11-1.5-1.11-1.5-.91-.64.07-.63.07-.63 1 .07 1.53 1.06 1.53 1.06.9 1.57 2.36 1.12 2.94.86.09-.67.35-1.12.63-1.38-2.22-.26-4.56-1.14-4.56-5.07 0-1.12.39-2.03 1.03-2.75-.1-.26-.45-1.3.1-2.71 0 0 .84-.27 2.75 1.05A9.3 9.3 0 0 1 12 6.84c.85 0 1.71.12 2.51.34 1.91-1.32 2.75-1.05 2.75-1.05.55 1.41.2 2.45.1 2.71.64.72 1.03 1.63 1.03 2.75 0 3.94-2.34 4.8-4.57 5.06.36.32.68.94.68 1.9 0 1.37-.01 2.47-.01 2.81 0 .27.18.6.69.49A10.03 10.03 0 0 0 22 12.26C22 6.58 17.52 2 12 2Z"
            />
          </svg>
          GitHub
        </a>

        <p className="home-credit">Made by {META.author}</p>
      </div>
    </section>
  )
}
